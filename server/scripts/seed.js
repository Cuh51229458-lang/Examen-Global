...
await mongoose.connect(
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27019/entrega",
);
const definiciones = [
  ["Desarrollo web", "#52745b"],
  ["Bases de datos", "#b8793c"],
  ["Diseño de interfaces", "#8070ad"],
  ["Redes", "#4e819a"],
];
const materias = [];
for (const [nombre, color] of definiciones)
  materias.push(
    await Materia.findOneAndUpdate(
      { clave: nombre.toLowerCase() },
      { $setOnInsert: { nombre, color } },
...
for (const [titulo, materia, dias, prioridad, estado, minutos] of ejemplos) {
  ...
  await Entrega.updateOne(
    { clave: titulo.toLowerCase(), materia: materias[materia]._id },
    {
      $setOnInsert: {
        titulo,
        descripcion:
          "Entrega de ejemplo para explorar el flujo académico. Puedes editarla o eliminarla.",
        fechaLimite,
        prioridad,
        estado,
        minutos,
...
console.log(
  "Datos de demostración creados en MongoDB sin reemplazar registros existentes.",
);
